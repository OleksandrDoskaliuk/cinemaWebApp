package ua.lviv.elearn.odoskaliuk.cinema.web;

public class Path {
	public static final String PAGE_SCHEDULE = "/WEB-INF/jsp/schedule.jsp";
	public static final String PAGE_HELLO = "/hello.jsp";
	public static final String PAGE_ALL_MOVIES = "/WEB-INF/jsp/admin/allMovies.jsp";
	public static final String PAGE_ADMIN = "/WEB-INF/jsp/admin/adminPage.jsp";
	public static final String PAGE_ADD_MOVIE_TO_SCHEDULE = "/WEB-INF/jsp/admin/addMovieToSchedule.jsp";
	public static final String PAGE_REMOVE_MOVIE_FROM_SCHEDULE = "/WEB-INF/jsp/admin/removeMovieFromSchedule.jsp";
	public static final String PAGE_CREATE_NEW_MOVIE = "/WEB-INF/jsp/admin/createNewMovie.jsp";
	
}
